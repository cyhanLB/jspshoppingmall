...
<%
	...
	if(option.equals("I")){
		...
		String query = "insert into tbl_m1reply (seqno,userid,replywriter,replycontent,replyregdate) "
					+ "values (" + seqno + ",'" + userid + "','" + replywriter + "','" + replycontent + "','" + replyregdate + "')";
	...
	if(option.equals("U")){
		...
		String query = "update tbl_m1reply set replycontent = '" + replycontent + "' where replyseqno =" + replyseqno;
	...
	if(option.equals("D")) {  
		...
		String query = "delete from tbl_m1reply where replyseqno = " + replyseqno;
	...
	try{
		...
		String query = "select replyseqno, replywriter, replycontent, to_char(replyregdate, 'YYYY-MM-DD HH24:MI:SS') "
				 	+ "as replyregdate, userid from tbl_m1reply where seqno = " + seqno 
		...
		while(rs.next()){
			...
			jsonObject.addProperty("userid",rs.getString("userid"));
